...
DOMAIN_PROFILES: dict[str, dict[str, Any]] = {
    "rag_facts": {
        "query_prefix": "fatos estaveis, enciclopedia, referencias externas e evidencias factuais sobre: ",
        "preferred_task_types": ["research", "general"],
        "top_k": 4,
        "threshold": 0.42,
    },
    "rag_code": {
        "query_prefix": "codigo, comandos, APIs, bibliotecas, funcoes, classes, SQL, Git, Docker e implementacao sobre: ",
        "preferred_task_types": ["coding", "operations"],
        "top_k": 4,
        "threshold": 0.30,
    },
    "rag_user_memory": {
        "query_prefix": "memoria do usuario, preferencias, fatos pessoais, continuidade da conversa e episodios sobre: ",
        "preferred_task_types": ["conversation_ptbr", "planning", "general"],
        "top_k": 3,
        "threshold": 0.26,
    },
    "rag_project_docs": {
        "query_prefix": "documentos do projeto, arquitetura, roadmap, endpoints, decisoes tecnicas e planos sobre: ",
        "preferred_task_types": ["planning", "research", "coding"],
        "top_k": 4,
        "threshold": 0.34,
    },
    "rag_self_model": {
        "query_prefix": "self-model, identidade do sistema, capacidades, limites, mudancas internas e estado cognitivo sobre: ",
        "preferred_task_types": ["conversation_ptbr", "planning", "general"],
        "top_k": 3,
        "threshold": 0.24,
    },
    "rag_runtime_logs": {
        "query_prefix": "logs de runtime, incidentes, latencia, health, workers, filas, erros e operacao sobre: ",
...
DOMAIN_ALIASES = {
    ...
    "facts": "rag_facts",
    "code": "rag_code",
    "memory": "rag_user_memory",
    "user_memory": "rag_user_memory",
    "planning": "rag_project_docs",
    "project_docs": "rag_project_docs",
    "self_model": "rag_self_model",
    "runtime": "rag_runtime_logs",
    "logs": "rag_runtime_logs",
    "safety": "rag_project_docs",
}

TASK_TO_DOMAINS = {
    "debug": ["rag_runtime_logs", "rag_code", "rag_project_docs"],
    "code": ["rag_code", "rag_project_docs", "rag_facts"],
    "coding": ["rag_code", "rag_project_docs", "rag_runtime_logs"],
    "summarization": ["rag_project_docs", "rag_facts"],
    "planning": ["rag_project_docs", "rag_user_memory", "rag_self_model"],
    "memory": ["rag_user_memory", "rag_project_docs"],
    "tool_action": ["rag_runtime_logs", "rag_code"],
    "self_model": ["rag_self_model", "rag_project_docs"],
    "general": ["rag_facts", "rag_project_docs"],
}


def normalize_domain(domain: str) -> str:
    d = str(domain or "").strip().lower()
    return DOMAIN_ALIASES.get(d, d if d in DOMAIN_PROFILES else "rag_facts")


def threshold_for(domain: str) -> float:
    profile = DOMAIN_PROFILES.get(normalize_domain(domain)) or DOMAIN_PROFILES["rag_facts"]
    return float(profile.get("threshold") or 0.0)


def infer_domains(query: str, task_type: str = "general") -> list[str]:
    q = str(query or "").lower()
    if any(k in q for k in ["documentacao", "documentação", "docs", "roadmap", "milestone", "arquitetura", "projeto", "decisao", "decisão", "planejar", "estrategia", "estratégia"]):
        domains = ["rag_project_docs", "rag_self_model"]
        if any(k in q for k in ["api", "endpoint", "git", "docker", "python", "npm", "sql", "codigo", "código"]):
            domains.insert(1, "rag_code")
        return domains
    if any(k in q for k in ["git", "docker", "python", "npm", "sql", "api", "endpoint", "classe", "funcao", "função", "commit", "stacktrace"]):
        domains = ["rag_code", "rag_project_docs"]
        if any(k in q for k in ["erro", "falha", "bug", "timeout", "latencia", "latência", "503", "502", "500"]):
            domains.insert(0, "rag_runtime_logs")
        return domains
    if any(k in q for k in ["erro", "falha", "bug", "traceback", "stack", "timeout", "latencia", "latência", "503", "502", "500", "log", "logs", "health", "worker"]):
        return ["rag_runtime_logs", "rag_code", "rag_project_docs"]
    if any(k in q for k in ["meu ", "minha ", "eu disse", "eu falei", "voce lembra", "você lembra", "vc lembra", "prefer", "gosto", "memoria do usuario", "memória do usuário"]):
        return ["rag_user_memory"]
    if any(k in q for k in ["quem e voce", "quem é você", "seu self", "self-model", "self model", "proprio sistema", "próprio sistema", "suas capacidades", "seus limites", "voce tem consciencia", "você tem consciência"]):
        return ["rag_self_model", "rag_project_docs"]
    if any(k in q for k in ["quem ", "qual ", "quando ", "onde ", "capital", "autor", "escreveu", "obra", "livro", "fato"]):
        return ["rag_facts"]
    return [normalize_domain(d) for d in TASK_TO_DOMAINS.get(str(task_type or "general"), ["rag_facts"])]
...
def _domain_key(doc: dict[str, Any]) -> str:
    return str(doc.get("domain") or doc.get("rag_type") or "unknown")
...
def _compute_selection_metrics(*, selected: list[dict[str, Any]], candidates: list[dict[str, Any]], requested_domains: list[str]) -> dict[str, Any]:
    ...
    domain_diversity = len(selected_domains) / max(1, len(candidate_domains or requested_domains or ["rag_facts"]))
    ...
    return {
        ...
        "candidate_sources": sorted(candidate_sources)[:12],
    }
...
async def search_routed(query: str, task_type: str = "general", top_k: int = 5, homeostasis_mode: str = "normal") -> dict[str, Any]:
    domains = [normalize_domain(d) for d in infer_domains(query=query, task_type=task_type)]
    max_domains = max(1, min(int(top_k or 5), int(os.getenv("ULTRON_RAG_MAX_DOMAINS", "2") or 2)))
    domains = domains[:max_domains]
    results: list[dict[str, Any]] = []
    search_plan: list[dict[str, Any]] = []
    threshold_rejections: list[dict[str, Any]] = []

    for domain in domains:
        profile = DOMAIN_PROFILES.get(domain) or DOMAIN_PROFILES["rag_facts"]
        ...
        for d in docs or []:
            item = dict(d or {})
            item["domain"] = domain
            item["rag_type"] = domain
            item["rag_threshold"] = threshold
            item["router_score"] = _score_doc(item, domain=domain, query=query)
            item["rag_threshold_passed"] = float(item.get("router_score") or 0.0) >= threshold
            if item["rag_threshold_passed"]:
                kept += 1
                results.append(item)
            else:
                threshold_rejections.append({
                    ...
                    "score": float(item.get("router_score") or 0.0),
                    "threshold": threshold,
                })
        search_plan.append({
            "domain": domain,
    ...
    return {
        "domains": domains,
        "rag_types": domains,
        "thresholds": {domain: threshold_for(domain) for domain in domains},
        "search_plan": search_plan,
        "diversity": diversity,
        "threshold_rejections": threshold_rejections[:12],
        "results": selected,
    }
